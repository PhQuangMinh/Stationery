...
const Checkout = () => {
    ...
    const handleSubmit = async (e) => {
        ...
        const selectedDistrictObj = districts.find(d => d.code === parseInt(selectedDistrict));
        const selectedWardObj = wards.find(w => w.code === parseInt(selectedWard));

        if (!selectedProvinceObj || !selectedDistrictObj || !selectedWardObj) {
            alert('Có lỗi xảy ra với thông tin địa chỉ. Vui lòng kiểm tra lại.');
            return;
        }
        ...
        try {
            ...
            // Gửi thông báo
            await fetch(`${BASE_API_URL}/user/notifications/send`, {
                method: 'POST',
                headers: {
                    'Content-Type': 'application/json',
                    'Authorization': `Bearer ${token}`
